<?php
if(!class_exists("EditorialRatingPROBase")) {
	class EditorialRatingPROBase {
    	public $key = "F1E00009D3E9822E";
    	private $product_id = "1";
    	private $product_base = "wpas_license";
    	private $server_host = "https://pluginic.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_wpas_license_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=wpas_license_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
