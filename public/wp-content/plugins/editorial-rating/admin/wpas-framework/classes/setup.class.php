...
if ( ! class_exists( 'WPAS_Setup' ) ) {
  class WPAS_Setup {
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( 'WPAS', 'setup' ) );
      add_action( 'init', array( 'WPAS', 'setup' ) );
      add_action( 'switch_theme', array( 'WPAS', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'WPAS', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'WPAS', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'WPAS', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'WPAS', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'WPAS_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPAS_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPAS_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPAS_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPAS_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPAS_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'WPAS_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( WPAS_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'WPAS_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPAS_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPAS_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'WPAS_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              WPAS_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'wpas_override', 'wpas-override' );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'wpas_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'WPAS_Field_'. $field ) && class_exists( 'WPAS_Fields' ) ) {
    ...
    public static function textdomain() {
      load_textdomain( 'wpas', self::$dir .'/languages/'. get_locale() .'.mo' );
    }
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! self::$enqueue ) {
        ...
        if ( $wpscreen->id === 'tools_page_wpas-welcome' ) {
          self::$enqueue = true;
        }

      }

      if ( ! apply_filters( 'wpas_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      if ( apply_filters( 'wpas_fa4', false ) ) {
        wp_enqueue_style( 'wpas-fa', 'https://cdn.jsdelivr.net/npm/font-awesome@4.7.0/css/font-awesome.min.css', array(), '4.7.0', 'all' );
      } else {
        wp_enqueue_style( 'wpas-fa5', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/all.min.css', array(), '5.15.5', 'all' );
        wp_enqueue_style( 'wpas-fa5-v4-shims', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/v4-shims.min.css', array(), '5.15.5', 'all' );
      }
      ...
      // Main style
      wp_enqueue_style( 'wpas', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );

      // Main RTL styles
      if ( is_rtl() ) {
        wp_enqueue_style( 'wpas-rtl', self::include_plugin_url( 'assets/css/style-rtl'. $min .'.css' ), array(), self::$version, 'all' );
      }

      // Main scripts
      wp_enqueue_script( 'wpas-plugins', self::include_plugin_url( 'assets/js/plugins'. $min .'.js' ), array(), self::$version, true );
      wp_enqueue_script( 'wpas', self::include_plugin_url( 'assets/js/main'. $min .'.js' ), array( 'wpas-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'wpas', 'wpas_vars', array(
        'color_palette'     => apply_filters( 'wpas_color_palette', array() ),
        'i18n'              => array(
          'confirm'         => esc_html__( 'Are you sure?', 'wpas' ),
          'typing_text'     => esc_html__( 'Please enter %s or more characters', 'wpas' ),
          'searching_text'  => esc_html__( 'Searching...', 'wpas' ),
          'no_results_text' => esc_html__( 'No results found.', 'wpas' ),
        ),
      ) );
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'WPAS_Field_' . $field['type'];
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['enqueue'] ) ) {
          ...
          wp_enqueue_style( 'wpas-google-web-fonts', esc_url( add_query_arg( $query, '//fonts.googleapis.com/css' ) ), array(), null );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'wpas' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      $is_pseudo  = ( ! empty( $field['pseudo'] ) ) ? ' wpas-pseudo-field' : '';
      ...
      if ( ! empty( $field['dependency'] ) ) {
        ...
        $visible = ( ! empty( $depend_visible ) ) ? ' wpas-depend-visible' : ' wpas-depend-hidden';

      }

      // These attributes has been sanitized above.
      echo '<div class="wpas-field wpas-field-'. $field_type . $is_pseudo . $class . $visible .'"'. $depend .'>';

      if ( ! empty( $field_type ) ) {

        if ( ! empty( $field['title'] ) ) {
          echo '<div class="wpas-title">';
          echo '<h4>'. $field['title'] .'</h4>';
          echo ( ! empty( $field['subtitle'] ) ) ? '<div class="wpas-subtitle-text">'. $field['subtitle'] .'</div>' : '';
          echo '</div>';
        }

        echo ( ! empty( $field['title'] ) ) ? '<div class="wpas-fieldset">' : '';

        $value = ( ! isset( $value ) && isset( $field['default'] ) ) ? $field['default'] : $value;
        $value = ( isset( $field['value'] ) ) ? $field['value'] : $value;

        $classname = 'WPAS_Field_'. $field_type;

        if ( class_exists( $classname ) ) {
          $instance = new $classname( $field, $value, $unique, $where, $parent );
          $instance->render();
        } else {
          echo '<p>'. esc_html__( 'Field not found!', 'wpas' ) .'</p>';
        }
