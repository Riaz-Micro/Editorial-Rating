 ...
 *
 */
if ( ! class_exists( 'WPAS_Field_spacing' ) ) {
  class WPAS_Field_spacing extends WPAS_Fields {
    ...
    public function render() {
      ...
      $is_unit = ( ! empty( $unit ) ) ? ' wpas--is-unit' : '';
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. $placeholder .' class="wpas-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( $unit ) ? '<span class="wpas--label wpas--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. $placeholder .' class="wpas-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="wpas--label wpas--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
