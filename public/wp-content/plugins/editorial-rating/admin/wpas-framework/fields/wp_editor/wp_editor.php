 ...
 *
 */
if ( ! class_exists( 'WPAS_Field_wp_editor' ) ) {
  class WPAS_Field_wp_editor extends WPAS_Fields {
    ...
    public function render() {
      ...
      echo ( wpas_wp_editor_api() ) ? '<div class="wpas-wp-editor" data-editor-settings="'. esc_attr( json_encode( $editor_settings ) ) .'">' : '';

      echo '<textarea name="'. esc_attr( $this->field_name() ) .'"'. $this->field_attributes( $attributes ) . $editor_height .'>'. $this->value .'</textarea>';

      echo ( wpas_wp_editor_api() ) ? '</div>' : '';
    ...
    public function enqueue() {
      ...
      if ( wpas_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var wpas_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( wpas_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'wpas_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'wpas_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'wpas_wp_editor', $setup );
