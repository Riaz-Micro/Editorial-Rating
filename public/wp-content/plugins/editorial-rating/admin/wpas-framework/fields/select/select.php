 ...
 *
 */
if ( ! class_exists( 'WPAS_Field_select' ) ) {
  class WPAS_Field_select extends WPAS_Fields {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( ! empty( $args['ajax'] ) ) {
          ...
          $args['settings']['data']['nonce'] = wp_create_nonce( 'wpas_chosen_ajax_nonce' );
        ...
        $chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' wpas-chosen-sortable' : '';
        $chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' wpas-chosen-ajax' : '';
        $placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="'. esc_attr( $args['placeholder'] ) .'"' : '';
        $field_class      = ( $args['chosen'] ) ? ' class="wpas-chosen'. esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) .'"' : '';
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select name="'. $field_name .'" class="wpas-hide-select hidden"'. $multiple_attr . $field_attr .'>';
          ...
          echo ( ! empty( $this->field['empty_message'] ) ) ? wp_kses_post( $this->field['empty_message'] ) : esc_html__( 'No data available.', 'wpas' );
