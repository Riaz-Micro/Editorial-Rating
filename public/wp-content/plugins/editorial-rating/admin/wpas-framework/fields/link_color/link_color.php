 ...
 *
 */
if ( ! class_exists( 'WPAS_Field_link_color' ) ) {
  class WPAS_Field_link_color extends WPAS_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'wpas' ),
        'hover'    => esc_html__( 'Hover', 'wpas' ),
        'active'   => esc_html__( 'Active', 'wpas' ),
        'visited'  => esc_html__( 'Visited', 'wpas' ),
        'focus'    => esc_html__( 'Focus', 'wpas' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="wpas--left wpas-field-color">';
          echo '<div class="wpas--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="wpas-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
