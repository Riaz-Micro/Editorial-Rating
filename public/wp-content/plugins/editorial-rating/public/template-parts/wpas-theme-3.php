...
// Get pros-cons label.
$wpas_pros_label = empty( $wpas_meta_values['wpas-pros-label'] ) ? 'ADVANTAGE' : $wpas_meta_values['wpas-pros-label'];
$wpas_cons_label = empty( $wpas_meta_values['wpas-cons-label'] ) ? 'DISADVANTAGE' : $wpas_meta_values['wpas-cons-label'];

// Enqueuing.
wp_enqueue_style( $this->plugin_name . '-theme-3' );
...
$wpas_heder_bg_color         = isset( $wpas_global_options['wpas-tabbed']['wpas-header-bg-color'] ) ? $wpas_global_options['wpas-tabbed']['wpas-header-bg-color'] : '';
$wpas_product_overview_color = isset( $wpas_global_options['wpas-tabbed']['wpas-product-overview-color'] ) ? $wpas_global_options['wpas-tabbed']['wpas-product-overview-color'] : '';
...
?>
<div id="wpas-er-<?php echo esc_attr( $wpas_post_id ); ?>" class="wpas--rating-wrap wpas-theme-3" data-id="<?php echo esc_attr( $wpas_post_id ); ?>" data-vote="<?php echo esc_attr( $frhd_total_vote ); ?>" data-star="<?php echo esc_attr( $frhd_total_star ); ?>" data-baranim="<?php echo esc_attr( $wpas_bar_animation_data ); ?>">
	...
	<div class="wpas--rating-header">
		...
		<div class="frhd--er-meta-container">
			<div class="frhd--reviewer">
				<span>Reviewed by <strong><a href="<?php echo esc_url( $wpas_get_author_link ); ?>"><?php echo esc_html( $wpas_get_author_name ); ?></a></strong></span>
				<br>
				<span><?php echo esc_html( $wpas_author_designation ); ?></span>
			</div>
			...
			<div class="frhd--user-rating">
				...
				<strong class="wpas--total-category"><span><?php echo esc_html( $frhd_rate_math_round ); ?>/5 by <?php echo esc_html( $frhd_total_vote ); ?> Users</span></strong>
			</div>
			<?php endif; ?>
	...
	<div class="wpas--rating-header-outers">
		<div class="wpas--rating-content">
			<div class="wpas--rating-desc">
				...
				?>
				<div class="wpas--rating-overview"><?php echo wp_kses_post( wpautop( $wpas_score_overview ) ); ?></div>
			</div>
					...
					if ( $wpas_editorial_rating_show ) {
						...
						foreach ( $wpas_editorial_rating as $wpas_editorial_rating_key => $wpas_editorial_rating_value ) {
							...
							echo '<div class="wpas--rating-detail">
								...
								<span class="wpas--rating-rate"><strong>' . esc_html( $wpas_editorial_rating_value['wpas-editorial-rating-category-rate'] ) . '</strong></span>
							</div>
							<div class="wpas--rating-meter">	
								<span class="wpas--rating-bar" style="width: ' . esc_html( round( $wpas_editorial_rating_value['wpas-editorial-rating-category-rate'] ) ) . '0%"></span>
							</div>';
						}
		...
		if ( $is_wpas_pros_cons_shows ) {
			...
			echo '<div class="wpas--pros-cons-wrap">
					<div class="wpas--pros-wrap">
						<h4 class="wpas--pros-title">' . $wpas_pros_label . '</h4>
			...
			foreach ( $wpas_wpas_pros as $wpas_wpas_pros_key => $wpas_wpas_pros_value ) {
				...
				echo '<li><span class="icon-checkmark"></span>' . $wpas_wpas_pros_value['wpas-pros-list'] . '</li>';
					...
					<div class="wpas--cons-wrap">
						<h4 class="wpas--cons-title">' . $wpas_cons_label . '</h4>
